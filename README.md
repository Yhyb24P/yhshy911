...
Q2/Q4 在 N=81、前 12 h 的积分通量检查中，95% 相对误差分别为 `8.81×10⁻⁶` 和 `7.51×10⁻⁶`。这证明当前经验含水率方程的离散积分与边界通量一致，不代表设备层面的真实水质量或能量闭合。

品质扫描以 4 h 后平台温度为控制量，同时最小化终止时间和终点面积加权标准差。程序不预设两者必然冲突，而是从数值结果提取非支配点，并报告最快、最均匀及归一化目标空间中的折中方案。可通过 `--hm-factors` 增加边界传质能力二维情景；由于缺少风速标定和设备功率数据，这些目标称为干燥效率与含水率均匀性，不称为风速方案或能耗最优。

N=321 的正式单参数扫描共包含 18 个情景。平台温度从 48 °C 升至 52 °C 时，终止时间由 61.3147 h 单调降至 53.9942 h，终点面积加权标准差由 0.0194299 轻微降至 0.0194249。在考察区间内，升温同时改善两个目标，没有形成速度–均匀性冲突，因而 Pareto 集仅含 52 °C 方案。相对实测平台 50.165 °C，52 °C 将终止时间缩短 3.1962 h（5.59%），而标准差仅下降 2.45×10⁻⁶（0.0126%）；可辨识的主要收益来自干燥效率，均匀性变化很弱。该结论只适用于当前平台温度区间和模型闭合假设。

统一可视化入口只读取冻结结果和派生分析文件，不重新求解 PDE。输出包括环境输入、Q1–Q4 时空热图、Q4 物理域/材料域双图、关键径向剖面、干燥速率、收敛性、敏感性及 Pareto 决策图。热图同时生成高分辨率 PNG 和带栅格化色块的 PDF，其坐标、文字与叠加曲线仍保持矢量。

`check_analysis.py` 会重算支配关系和事件约束，检查每个方案的均值、标准差、极差及 101 点终点剖面，并通过 SHA-256 核对品质 CSV 和全部 27 个图件的实际内容。
...
│   ├── validate_metrics.py     # 平均含水率积分通量检查
│   ├── check_analysis.py       # 品质/Pareto/图件内容指纹审计
