...
热风烘干需要在给定过程环境下预测药材内部的升温与脱水进程，并以统一的
终止判据量化固定尺寸和实测收缩尺寸下的干燥时长。为降低试验迭代成本，
本文以圆柱药材的径向温湿场为对象，建立可复现的时变边界数值模型
\cite{simal1998,mujumdar2014}。
...
\noindent\textbf{已知参数与共同约束。}药材初始长度 $L=25\ \mathrm{cm}$、半径
$R_0=2\ \mathrm{cm}$、温度 $T_0=28\ ^\circ\mathrm{C}$、干基含水率
$X_0=2.55\ \mathrm{kg\,kg^{-1}}$。附件 1 给出 $T_a(t)$ 与外部有效水分势
$\chi_a(t)$，附件 2 给出实测 $R(t)$；附录 2--4 分别给出相应物性。所有问题
均以 $T(r,t)$、$X(r,t)$ 为状态，在一维径向域内求解，数值结果保留四位小数
\cite{cumcm2026}。

\noindent\textbf{问题 1（预热）。}在 $0\le r\le R_0$ 与附录 2 物性下，求
$(r,t)\mapsto[T,X]$；给出 $r=0,0.5,1,1.5,2\ \mathrm{cm}$、
$t=100,300,\ldots,1800\ \mathrm{s}$ 的截面及 $1\ \mathrm{s}\times0.1\ \mathrm{cm}$
离散场。

\noindent\textbf{问题 2（变物性固定域）。}保持 $R_0$，以附录 3 的
$\rho(X),c_p(X),k(X),D_{\mathrm{eff}}(X,\Theta)$ 更新状态方程；报告
$0$--$3\ \mathrm{h}$ 内每 $0.5\ \mathrm{h}$ 和五个指定半径的结果。

\noindent\textbf{问题 3（固定域终止）。}延拓问题 2 的过程，求连续事件
\[
t_{\mathrm{dry}}=\inf\{t>0:\max_{0\le r\le R_0}X(r,t)\le0.15\},
\]
并将每 $6\ \mathrm{h}$ 的代表性剖面与连续事件时间分别报告。

\noindent\textbf{问题 4（实测收缩域）。}令 $0\le r\le R(t)$，在
$\xi=r/R(t)$ 上采用附录 4 物性重求同一终止事件；固定物理位置一旦越出
收缩域即留空，表面量单独按 $r=R(t)$ 输出。

% ==================== 二、问题分析 ====================
\section{问题分析}

四问不是相互独立的计算任务，而是沿“短时固定域场--长时非线性场--
全域终止事件--实测收缩移动域”逐层增加时间尺度、物性非线性、输出约束
与几何条件的同一热湿传递问题。药材长径比为 $25/4\approx6.25$，端面总
面积仅约为侧面积的 $8\%$；结合周向边界近似均匀，采用一维轴对称径向
描述可保留主导梯度\cite{hussain2003}。共同未知量为温度 $T(r,t)$ 与干基
含水率 $X(r,t)$；附件 1 的 $T_a(t),\chi_a(t)$ 经 Robin 边界进入模型，
而附录物性将二者参数耦合。

图 \ref{fig:route} 将本地数据、四问模型链、数值算法、约束输出及独立校验
连接为一条可追溯路线。实线表示数据或计算流；虚线只把冻结的正式结果送入
验证和情景分析，不反馈改写生产求解器。
...
\noindent\textbf{问题 1：短时强边界层的非稳态场解析。}
该问本质是时变 Robin 边界下的固定半径热湿初值问题：已知 $R_0$、附录 2
常物性及附件 1 的环境历程，需求解前 $1800\ \mathrm{s}$ 的 $T,X$ 径向场。
难点不在终止判定，而在 $t=1\ \mathrm{s}$ 形成的薄表面水分层；粗网格会把
局部梯度误写成整体脱水。故采用单元中心有限体积离散、Backward Euler 启动
与变步长 BDF2 推进，以 $N=2561$ 和最大步长 $0.03125\ \mathrm{s}$ 捕捉该
瞬态；再以表面水分的时空加密试验检验离散分辨率，而非仅比较终点数值。

\noindent\textbf{问题 2：由问题 1 扩展的长时变系数非线性场。}
该问保持固定几何和同一通量形式，却将常物性替换为附录 3 的
$\rho(X),c_p(X),k(X),D_{\mathrm{eff}}(X,\Theta)$，因此本质上是非线性
变系数抛物型场计算。温湿状态既决定物性又被物性改变，直接显式推进会受
稳定性限制；故每个时间层用 Picard 迭代冻结并更新系数，界面导热与扩散
系数取调和平均，并以半单元阻力与表面对流阻力串联处理 Robin 条件。该链
从问题 1 的短时网格控制延伸至全过程，同时以 $0$--$3\ \mathrm{h}$ 截面、
全字段工作簿和收敛结果共同核验。

\noindent\textbf{问题 3：在问题 2 场解上的连续约束首次穿越。}
该问不另建 PDE，而将问题 2 的固定域解转化为全域不等式约束的首次满足
问题。定义 $g(t)=\max_{0\le r\le R_0}X(r,t)-0.15$；未知量是
$g(t)=0$ 的首次根，不能把规则 $60\ \mathrm{s}$ 输出行当作根。时间推进时
先括住相邻步的符号变化，再在该区间二分定位连续事件；最后将事件前后
$g(t)$ 的符号、$60\ \mathrm{s}$ 与 $15\ \mathrm{s}$ 最大步长比较及代表性
剖面作为准确性检查。

\noindent\textbf{问题 4：在问题 3 约束上引入外部给定的移动几何。}
该问的增量是附件 2 给定的 $R(t)$，而非由水分场反推收缩；因此它是实测
几何单向驱动的移动域问题。先以 PCHIP 对半径观测值保形插值，再取材料
坐标 $\xi=r/R(t)$ 将 $[0,R(t)]$ 映射为 $[0,1]$，沿用有限体积时间推进与
事件根定位，并切换为附录 4 物性。该构造避免重构物理网格；以越界固定
位置留空、表面单列和同一 $g(t)$ 终止判据校验输出语义。PCHIP 的保形性
避免普通高阶插值在实测收缩序列中产生额外振荡\cite{fritsch1980}。

\noindent\textbf{独立校验与工艺比较。}
正式输出冻结后，Bessel/Duhamel 基准、空间--时间收敛、工作簿审计和通量
闭合分别检查方程、离散、交付与数值一致性；敏感性和平台温度扫描只消费
这些结果。其评价指标为 $t_{\mathrm{dry}}$ 与终点含水率均匀性；由于当前
模型不含品质降解和能耗机理，温度扫描仅给出考察区间内的条件性比较，
不宣称生产全局最优。
...
\begin{enumerate}[label=\textbf{Step \arabic*:}, leftmargin=2.6cm]
  ...
  \item 将生产结果冻结后，运行解析基准、空间/时间收敛、通量闭合与工作簿
